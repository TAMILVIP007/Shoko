...
from Shoko import dispatcher, LOGGER, OWNER_ID, MESSAGE_DUMP
from Shoko.__main__ import DATA_IMPORT
...
# from Shoko.modules.rules import get_rules
import Shoko.modules.sql.rules_sql as rulessql

# from Shoko.modules.sql import warns_sql as warnssql
import Shoko.modules.sql.blacklist_sql as blacklistsql
from Shoko.modules.sql import disable_sql as disabledsql

# from Shoko.modules.sql import cust_filters_sql as filtersql
# import Shoko.modules.sql.welcome_sql as welcsql
import Shoko.modules.sql.locks_sql as locksql
...
def export_data(update, context):
    ...
    f = open("Shoko{}.backup".format(chat_id), "w")
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("Shoko{}.backup".format(chat_id), "rb"),
        caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `Shoko-Backup` is specially made for notes.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("Shoko{}.backup".format(chat_id))  # Cleaning file
